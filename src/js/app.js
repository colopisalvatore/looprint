    ...
    // Using CounterAPI V1 (official, no auth required) + localStorage caching for maximum reliability
    // CounterAPI V1: https://docs.counterapi.dev/api/endpoints/v1/
    // - No authentication required
    // - Public counters
    // - Rate limit: 30 requests/minute/IP (sufficient for our use case)
    const COUNTER_NAMESPACE = 'looprint';
    const COUNTER_KEY = 'total-loops';
    const COUNTER_API_BASE = 'https://api.counterapi.dev/v1';
    const COUNTER_STORAGE_KEY = 'looprint_global_counter_cache'; // localStorage key for caching 
...
`;
    ...
    const LANG = {
        en: { 
            ...
            validationWarning: "✓ Push direction is automatically detected from model placement.", validationError: "Validation failed: Could not read or parse the file. Please ensure it is a valid G-code or 3MF file.",
            ...
            validationSuccess: "File verified: Valid G-code or 3MF file detected. Start and end code will be added automatically.",
            validationSuccessGcodeFactorian: "✓ File verified: G-code with Factorian Designs templates detected. Start and end code will be added automatically.",
            validationSuccessGcodeBambu: "✓ File verified: G-code from Bambu Studio detected. Start and end code will be added automatically.",
            validationSuccessGcodeStandard: "✓ File verified: G-code file detected. Start and end code will be added automatically.",
            validationSuccess3MF: "✓ File verified: 3MF file detected. Start and end code will be added automatically.", step2Title: "2. Loop Configuration", loopsLabel: "Number of loops", safetyText: "I confirm this file was sliced with Brim enabled.",
            ...
            zOffsetLabel: "Z Push Offset (mm)", zOffsetApply: "Apply", zOffsetHelp: "Default 30mm below top (P1). A1 Factorian logic uses 40mm in the automated calculation.", zOffsetWarning: "⚠️ Advanced: Incorrect values can cause collisions.",
            ...
            debugHeaderTitle: "📍 Model Placement Analysis:",
...
`;
    ...
    function updateDirectionInfo() {
        ...
        if (currentDirection === autoDetectedDirection) {
            ...
            const icon = document.createTextNode("ℹ️ ");
    ...
    function updateFileSizeEstimate() {
        ...
        const text = document.createTextNode(`~${formatFileSize(estimatedWithOverhead)} (${loopCount} loops × ${formatFileSize(fileSize)})`);
    ...
    /**
     * Initialize global counter and load current value
     * Uses CountAPI.xyz which is free and doesn't require authentication
     */
    ...
    /**
     * Load current counter value with fallback strategy
     * 1. Try to fetch from API
     * 2. If API fails, use cached value from localStorage
     * 3. If no cache, show 0
     */
    async function loadGlobalCounter() {
        const counterEl = document.getElementById('global-counter');
        if (!counterEl) {
            console.warn('Counter UI element not found');
            return;
        }
        ...
        // First, try to load cached value from localStorage as fallback
        let cachedCount = null;
        try {
            const cached = localStorage.getItem(COUNTER_STORAGE_KEY);
            if (cached) {
                cachedCount = parseInt(cached);
                if (!isNaN(cachedCount) && cachedCount >= 0) {
                    // Show cached value immediately while we fetch from API
                    updateCounterUI(cachedCount);
                }
            }
        } catch (e) {
        ...
        try {
            // CounterAPI V1 endpoint: GET /v1/{namespace}/{name}/
            // Format: https://api.counterapi.dev/v1/{namespace}/{name}/
            const url = `${COUNTER_API_BASE}/${COUNTER_NAMESPACE}/${COUNTER_KEY}/`;
            ...
            if (!response.ok) {
                // If counter doesn't exist yet (404), it will be created on first increment
                if (response.status === 404 || response.status === 400) {
                    console.log('Counter not found - will be created on first increment');
                    // Use cached value if available, otherwise show 0
                    if (cachedCount !== null && !isNaN(cachedCount)) {
                        updateCounterUI(cachedCount);
                    } else {
                        updateCounterUI('0');
                    }
            ...
            // CounterAPI V1 returns { value: number } on success
            // Documentation: https://docs.counterapi.dev/api/endpoints/v1/
            let count = null;
            if (data) {
            ...
            if (count !== null && !isNaN(count) && count >= 0) {
                // Successfully loaded from API - update UI and cache
            ...
            console.warn('Failed to load global counter from API:', error);
            // Use cached value if available, otherwise show 0
            if (cachedCount !== null && !isNaN(cachedCount)) {
                updateCounterUI(cachedCount);
            } else {
    ...
    /**
     * Increment global counter by specified amount
     * Uses CounterAPI V1 (official, no auth) with localStorage caching for maximum reliability
     * Automatically creates counter if it doesn't exist
     * Note: CounterAPI V1 only supports +1 per call, so we use optimistic updates for the full amount
     * @param {number} loops - Number of loops to add to counter
     */
    async function incrementGlobalCounter(loops) {
        if (!loops || loops < 1) {
            console.warn('Invalid loops value for counter:', loops);
            return;
        }
        
        // Get current count from UI to use as fallback
        let currentCount = 0;
        const counterEl = document.getElementById('global-counter');
        if (counterEl && counterEl.textContent !== '...' && counterEl.textContent !== 'N/A') {
            const currentText = counterEl.textContent.replace(/,/g, '');
        ...
        try {
            const cached = localStorage.getItem(COUNTER_STORAGE_KEY);
            if (cached) {
                const cachedCount = parseInt(cached);
                if (!isNaN(cachedCount) && cachedCount > currentCount) {
                    // Use cached value if it's higher (more recent)
        ...
        const optimisticCount = currentCount + loops;
        updateCounterUI(optimisticCount);
        
        // Also update localStorage cache immediately
        try {
            localStorage.setItem(COUNTER_STORAGE_KEY, optimisticCount.toString());
        } catch (e) {
            // localStorage might not be available
        }
        
        // Now try to update on server (non-blocking)
        // CounterAPI V1 automatically creates counter on first increment
        // Note: CounterAPI V1 only supports +1 per call (no custom increment amounts)
        // We'll call it multiple times (up to loops times) but limit to avoid rate limits
        // Rate limit: 30 requests/minute/IP - we'll be conservative and call max 5 times
        // The optimistic update already shows the correct total, so this is just for server sync
        try {
            const maxIncrementCalls = Math.min(loops, 5); // Limit to 5 calls to avoid rate limits
            let lastServerCount = currentCount;
            
            // Call increment endpoint multiple times (but limited)
            for (let i = 0; i < maxIncrementCalls; i++) {
                const url = `${COUNTER_API_BASE}/${COUNTER_NAMESPACE}/${COUNTER_KEY}/up`;
                const response = await fetch(url, {
                    method: 'GET',
                    cache: 'no-cache',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    // Rate limit hit - stop calling
                    console.log('Rate limit reached, stopping increment calls');
                    break;
                } else if (response.status === 404 || response.status === 400) {
                    // Counter doesn't exist yet - first call will create it
                ...
                if (i < maxIncrementCalls - 1) {
                    await new Promise(resolve => setTimeout(resolve, 100));
                }
            }
            
            // Update cache with server value if it's meaningful
            // But keep optimistic value in UI since it's more accurate (includes all loops)
            if (lastServerCount > currentCount) {
                try {
                    // Store the server value, but UI already shows optimistic (correct) value
                    localStorage.setItem(COUNTER_STORAGE_KEY, optimisticCount.toString());
                } catch (e) {
                    // localStorage might not be available
                }
            ...
            // Silently fail - don't block generation or show errors
            // We've already updated UI optimistically, so this is fine
            console.warn('Failed to increment global counter on server:', error);
        }
    }

    /**
     * Update counter UI with formatted number
     * @param {number|string} value - Counter value or error string
     */
    ...
    function saveSettings() {
        try {
            ...
            if (speedModeInput) {
                localStorage.setItem('looprint_speedMode', speedModeInput.value);
            }
        } catch (e) {
            // localStorage might not be available (e.g., private browsing mode)
    ...
    function determineModelPlacement(gcode) {
        ...
        // For store filer: Parse linje-for-linje i stedet for kompleks regex på hele filen
        ...
        try {
            ...
            for (let i = 0; i < MAX_LINES; i++) {
                ...
                // 1. Z-højden er over threshold (langt over start purge)
                // 2. E-værdien er positiv (det er en reel ekstrudering/printlinje), uanset Z-højde
    ...
    // --- SLUT PÅ determineModelPlacement() FUNKTION ---
    ...
    function updateFooterText(is3MF) {
        ...
        if (footerTextEl) {
            ...
            // Text format: "<strong>⚠️ Safety reminder:</strong> Always test..."
                ...
                try {
                    ...
                    if (detectedPrinterModel) {
                        ...
                        if (modelSelect) {
                            ...
                            showStatus("validationWarning", "success", `✓ Printer model auto-detected: ${detectedModelName}. Selector locked to match file.`);
                        }
                    } else {
                    ...
                    const dict = LANG.en;
                    statusEl.textContent = dict["validationSuccess3MF"] || "File verified: Valid 3MF file detected. Start and end code will be added automatically.";
        ...
        reader.onload = function(e) {
            try {
                ...
                if (hasLooprint) {
                    ...
                    // CRITICAL: Check if file has Factorian structure (required for Looprint to work)
                    // Factorian files MUST have at least one of these primary indicators:
                    // 1. EXECUTABLE_BLOCK_START + "Made by FactorianDesigns" or "FactorianDesigns" signature
                    // 2. FEATURE: Custom + "Made by FactorianDesigns" or "FactorianDesigns" signature
                    // 3. ;===== machine: P1S/X1/A1 with "Made by FactorianDesigns" or "FactorianDesigns" signature
                    // 
                    // NOTE: BambuStudio files also have EXECUTABLE_BLOCK_START and FEATURE: Custom,
                    // but they DON'T have "Made by FactorianDesigns" signature
                    const hasExecutableBlock = content.indexOf('; EXECUTABLE_BLOCK_START') !== -1;
                    const hasFeatureCustom = content.indexOf('; FEATURE: Custom') !== -1;
                    
                    // CRITICAL: Check for Factorian signature to distinguish from BambuStudio files
                    // BambuStudio files have EXECUTABLE_BLOCK_START and FEATURE: Custom but NOT "Made by FactorianDesigns"
                    const hasFactorianSignature = content.indexOf('Made by FactorianDesigns') !== -1 || 
                                                   content.indexOf('FactorianDesigns') !== -1;
                    
                    // Must match Factorian's exact format: ;===== machine: P1S ===== (with ====)
                    // CRITICAL: ;===== and machine: must be on the SAME LINE (Factorian format)
                    // BUT: Also check that Factorian signature is nearby (within 500 chars) to ensure it's the real start code
                    const factorianMachinePattern = /;=====\s+machine:\s*(P1S|P1|X1|X1C|A1)/i;
                    let hasFactorianMachine = false;
                    const machineMatch = content.match(factorianMachinePattern);
                    if (machineMatch) {
                        // Check if Factorian signature is nearby (within 500 chars after the match)
                        const matchIndex = machineMatch.index;
                        const nearbySection = content.substring(matchIndex, Math.min(matchIndex + 500, content.length));
                        const hasSignatureNearby = nearbySection.indexOf('Made by FactorianDesigns') !== -1 || 
                                                     nearbySection.indexOf('FactorianDesigns') !== -1;
                        hasFactorianMachine = hasSignatureNearby;
                    }
                    
                    // Primary checks - must have Factorian signature if using EXECUTABLE_BLOCK_START or FEATURE: Custom
                    // OR must have ;===== machine: format WITH Factorian signature nearby
                    let hasFactorianStructure = false;
                    if (hasFactorianMachine) {
                        // ;===== machine: format WITH Factorian signature nearby is unique to Factorian
                        hasFactorianStructure = true;
                    } else if (hasExecutableBlock || hasFeatureCustom) {
                        // If file has EXECUTABLE_BLOCK_START or FEATURE: Custom, must also have Factorian signature
                        hasFactorianStructure = hasFactorianSignature;
                    }
                    
                    // DEBUG: Always log validation results
                    if (DEBUG_MODE || !hasFactorianStructure) {
                        console.log("[VALIDATION] Checking Factorian structure:");
                        console.log("  - EXECUTABLE_BLOCK_START:", hasExecutableBlock);
                        console.log("  - FEATURE: Custom:", hasFeatureCustom);
                        console.log("  - ;===== machine: P1S/X1/A1:", hasFactorianMachine);
                        console.log("  - FactorianDesigns signature:", hasFactorianSignature);
                        if (machineMatch) {
                            console.log("  - ;===== machine: match found:", machineMatch[0].substring(0, 100));
                            const matchIndex = machineMatch.index;
                            const nearbySection = content.substring(matchIndex, Math.min(matchIndex + 500, content.length));
                            const hasSignatureNearby = nearbySection.indexOf('Made by FactorianDesigns') !== -1 || 
                                                         nearbySection.indexOf('FactorianDesigns') !== -1;
                            console.log("  - Factorian signature nearby:", hasSignatureNearby);
                            if (!hasSignatureNearby) {
                                console.warn("  - WARNING: ;===== machine: found but NO Factorian signature nearby - likely not a Factorian start code!");
                            }
                        }
                        console.log("  - hasFactorianStructure:", hasFactorianStructure);
                        if ((hasExecutableBlock || hasFeatureCustom) && !hasFactorianSignature) {
                            console.warn("  - WARNING: File has EXECUTABLE_BLOCK_START or FEATURE: Custom but NO FactorianDesigns signature - likely a BambuStudio file!");
                        }
                    }
                    
                    // Note: Looprint can handle files without Factorian structure by adding start/end code automatically
                    // Set appropriate validation message based on file type
                    if (hasFactorianStructure) {
                        // G-code file has Factorian structure
                        validationKey = "validationSuccessGcodeFactorian";
                        if (DEBUG_MODE) {
                            console.log("[VALIDATION] G-code file has Factorian structure");
                        }
                    } else if (hasExecutableBlock || hasFeatureCustom) {
                        // G-code from Bambu Studio (has EXECUTABLE_BLOCK_START or FEATURE: Custom but no Factorian signature)
                        validationKey = "validationSuccessGcodeBambu";
                        if (DEBUG_MODE) {
                            console.log("[VALIDATION] G-code file from Bambu Studio detected (no Factorian structure)");
                        }
                    } else {
                        // Standard G-code file (no special structure)
                        validationKey = "validationSuccessGcodeStandard";
                        if (DEBUG_MODE) {
                            console.log("[VALIDATION] Standard G-code file detected (no Factorian structure)");
                        }
                    }
                    validationColor = "var(--accent)";
                    
                    // CRITICAL: Detect printer model from G-code header FIRST (before placement detection)
                    updateProgress(50, "Detecting printer model from G-code...");
                    let detectedPrinterModel = null;
                    
                    // Check if file is already processed by Looprint (has Looprint signature)
                    const isLooprintFile = content.indexOf(LOOPRINT_SIGNATURE) !== -1 ||
                                          content.indexOf("Generated by Looprint") !== -1 ||
                                          content.indexOf("Looprint Factorian Loop File") !== -1;
                    
                    if (isLooprintFile && DEBUG_MODE) {
                        console.warn("[PRINTER MODEL DEBUG] File appears to be already processed by Looprint. Original machine header may not be present.");
                    }
                    
                    // Try to detect from G-code header comments (e.g., ";===== machine: P1S =====")
                    // Check first 5000 characters for header information (increased to catch headers deeper in processed files)
                    const headerSection = content.substring(0, Math.min(5000, content.length));
                    
                    // Also check CONFIG_BLOCK and EXECUTABLE_BLOCK for printer model info (BambuStudio format)
                    const configBlockStart = content.indexOf('; CONFIG_BLOCK_START');
                    const configBlockEnd = content.indexOf('; CONFIG_BLOCK_END');
                    let configBlockSection = '';
                    if (configBlockStart !== -1 && configBlockEnd !== -1) {
                        configBlockSection = content.substring(configBlockStart, Math.min(configBlockEnd + 200, content.length));
                    }
                    
                    const executableBlockStart = content.indexOf('; EXECUTABLE_BLOCK_START');
                    let executableBlockSection = '';
                    if (executableBlockStart !== -1) {
                        executableBlockSection = content.substring(executableBlockStart, Math.min(executableBlockStart + 10000, content.length));
                    }
                    
                    // Combine all sections to search
                    const searchSection = headerSection + '\n' + configBlockSection + '\n' + executableBlockSection;
                    
                    // CRITICAL: Check most specific models FIRST (P1S, X1C) to prevent false positives
                    // Pattern 1a: ;===== machine: P1S ===== (most specific - check first)
                    if (searchSection.match(/;=====.*machine:\s*P1S/i) ||
                        searchSection.match(/machine:\s*P1S/i)) {
                        detectedPrinterModel = 'P1';
                        if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected P1S from G-code");
                    }
                    // Pattern 2a: ;===== machine: X1C ===== (most specific - check before X1)
                    else if (searchSection.match(/;=====.*machine:\s*X1C/i) ||
                             searchSection.match(/machine:\s*X1C/i)) {
                        detectedPrinterModel = 'x1';
                        if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected X1C from G-code");
                    }
                    // Pattern 2b: ;===== machine: X1 ===== (check after X1C)
                    else if (searchSection.match(/;=====.*machine:\s*X1\b/i) ||
                             searchSection.match(/machine:\s*X1\b/i)) {
                        detectedPrinterModel = 'x1';
                        if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected X1 from G-code");
                    }
                    // Pattern 1b: ;===== machine: P1 ===== (check after P1S and X1/X1C to prevent false positives)
                    else if (searchSection.match(/;=====.*machine:\s*P1\b/i) || 
                             searchSection.match(/machine:\s*P1\b/i)) {
                        detectedPrinterModel = 'P1';
                        if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected P1 from G-code");
                    }
                    // Pattern 3: ;===== machine: A1 =====
                    else if (searchSection.match(/;=====.*machine:\s*A1\s*(?:mini)?/i) ||
                             searchSection.match(/machine:\s*A1\s*(?:mini)?/i)) {
                        // Check if it's A1 Mini specifically
                        if (searchSection.match(/machine:\s*A1\s+mini/i)) {
                            detectedPrinterModel = 'A1Mini';
                            if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected A1 Mini from G-code");
                        } else {
                            detectedPrinterModel = 'A1';
                            if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected A1 from G-code");
                        }
                    }
                    
                    // Fallback: Try to detect from CONFIG_BLOCK (Bambu Studio format)
                    // This is needed for Bambu Studio files that don't have Factorian format
                    if (!detectedPrinterModel && configBlockSection) {
                        if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] Trying CONFIG_BLOCK detection for Bambu Studio file...");
                        
                        // Look for printer_model = "Bambu Lab P1P" format (G-code files use this)
                        const printerModelMatch = configBlockSection.match(/printer_model\s*=\s*["']?([^"'\n]+)/i);
                        if (printerModelMatch) {
                            const modelName = printerModelMatch[1].trim();
                            if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] Found printer_model in CONFIG_BLOCK:", modelName);
                            
                            // Check for specific models in the name
                            if (modelName.match(/P1P|P1S|P1\b/i)) {
                                detectedPrinterModel = 'P1';
                                if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected P1/P1S/P1P from CONFIG_BLOCK printer_model");
                            } else if (modelName.match(/X1C|X1\b/i)) {
                                detectedPrinterModel = 'x1';
                                if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected X1/X1C from CONFIG_BLOCK printer_model");
                            } else if (modelName.match(/A1\s+mini|A1Mini/i)) {
                                detectedPrinterModel = 'A1Mini';
                                if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected A1 Mini from CONFIG_BLOCK printer_model");
                            } else if (modelName.match(/A1\b/i)) {
                                detectedPrinterModel = 'A1';
                                if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected A1 from CONFIG_BLOCK printer_model");
                            }
                        }
                        
                        // Also check print_compatible_printers (backup method)
                        if (!detectedPrinterModel) {
                            const compatiblePrintersMatch = configBlockSection.match(/print_compatible_printers\s*=\s*["']([^"']+)["']/i);
                            if (compatiblePrintersMatch) {
                                const compatiblePrinters = compatiblePrintersMatch[1];
                                if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] Found print_compatible_printers:", compatiblePrinters);
                                
                                if (compatiblePrinters.match(/P1P|P1S|P1\b/i)) {
                                    detectedPrinterModel = 'P1';
                                    if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected P1/P1S/P1P from print_compatible_printers");
                                } else if (compatiblePrinters.match(/X1C|X1\b/i)) {
                                    detectedPrinterModel = 'x1';
                                    if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected X1/X1C from print_compatible_printers");
                                } else if (compatiblePrinters.match(/A1\s+mini|A1Mini/i)) {
                                    detectedPrinterModel = 'A1Mini';
                                    if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected A1 Mini from print_compatible_printers");
                                } else if (compatiblePrinters.match(/A1\b/i)) {
                                    detectedPrinterModel = 'A1';
                                    if (DEBUG_MODE) console.log("[PRINTER MODEL DEBUG] ✓ Detected A1 from print_compatible_printers");
                                }
                            }
                        }
                    }
                    
                    // If printer model was detected, auto-select and lock it
                    if (detectedPrinterModel) {
                        const modelSelect = document.getElementById("printerModel");
                        const autoDetectedNote = document.getElementById("autoDetectedPrinterNote");
                        ...
                        if (modelSelect) {
                            ...
                            currentPrinterModel = detectedPrinterModel;
                            modelSelect.value = detectedPrinterModel;
                            modelSelect.disabled = true; // Lock the selector
                            
                            if (autoDetectedNote) {
                                const detectedModelName = detectedPrinterModel === 'A1' ? 'Bambu Lab A1' : 
                                                         (detectedPrinterModel === 'A1Mini' ? 'Bambu Lab A1 Mini' : 
                                                         (detectedPrinterModel === 'x1' ? 'Bambu Lab X1/X1C' : 'Bambu Lab P1/P1S'));
                                autoDetectedNote.textContent = `✓ Printer model auto-detected: ${detectedModelName}`;
                                autoDetectedNote.style.display = 'block';
                            }
                            
                            if (DEBUG_MODE) {
                                console.log("[PRINTER MODEL DEBUG] ✓ Auto-detected printer model from G-code:", detectedPrinterModel);
                                console.log("[PRINTER MODEL DEBUG] Header section (first 500 chars):", headerSection.substring(0, 500));
                                console.log("[PRINTER MODEL DEBUG] Model selector locked to:", detectedPrinterModel);
                            }
                            showStatus("validationWarning", "success", `✓ Printer model auto-detected: ${detectedPrinterModel === 'A1' ? 'Bambu Lab A1' : (detectedPrinterModel === 'A1Mini' ? 'Bambu Lab A1 Mini' : (detectedPrinterModel === 'x1' ? 'Bambu Lab X1/X1C' : 'Bambu Lab P1/P1S'))}. Selector locked to match file.`);
                        }
                    } else {
                        // No printer model detected - unlock selector and use default (P1)
                        autoDetectedPrinterModel = null;
                        const modelSelect = document.getElementById("printerModel");
                        const autoDetectedNote = document.getElementById("autoDetectedPrinterNote");
                        if (modelSelect) {
                        ...
                        if (DEBUG_MODE) {
                            console.warn("[PRINTER MODEL DEBUG] ✗ Could not detect printer model from G-code header");
                            console.log("[PRINTER MODEL DEBUG] Header section searched (first 500 chars):", headerSection.substring(0, 500));
                            console.log("[PRINTER MODEL DEBUG] Patterns checked: 'machine: P1S', 'machine: P1', 'machine: X1', 'machine: X1C', 'machine: A1', 'machine: A1 mini'");
                            console.log("[PRINTER MODEL DEBUG] Using manual selection (currentPrinterModel:", currentPrinterModel, ")");
                        }
                    }
    ...
    function resetDefaults() {
        ...
        // Reset Speed Mode to default (Standard 100%)
        const speedModeInput = document.getElementById("speedMode");
        if (speedModeInput) {
            speedModeInput.value = "100";
        }
        ...
        try {
            ...
            localStorage.removeItem('looprint_speedMode');
        } catch (e) {
            // localStorage might not be available (e.g., private browsing mode)
    ...
    async function processGcodeFile(file, loops) {
        ...
        reader.onload = async function(e) {
            try {
                ...
                // Extract print G-code only (remove existing end code) - MUST be done early
                ...
                // Parse G-code structure EARLY so gcodeConfig is available for variable extraction
                ...
                // Get end code element FIRST so activeEndCode is available for variable replacement
                let endCodeElement;
                if (currentPrinterModel === 'A1' || currentPrinterModel === 'A1Mini') {
                    FIREWALL_A1_A1MINI_ONLY('processGcodeFile: end code selection');
                    endCodeElement = document.getElementById('endA1Code');
                } else {
                    // P1/X1: Use direction-based end code elements (both use same push system)
                    FIREWALL_P1_X1_ONLY('processGcodeFile: end code selection');
                ...
                if (DEBUG_MODE) console.log("[DIAG] processGcodeFile: Extracted variable values:", Object.keys(endCodeVariableValues));
                if (currentPrinterModel === 'A1' || currentPrinterModel === 'A1Mini') {
                    if (DEBUG_MODE) console.log("[DIAG] processGcodeFile:", currentPrinterModel, "- endCodeVariableValues['first_layer_center_no_wipe_tower']:", endCodeVariableValues['first_layer_center_no_wipe_tower']);
                }
                ...
                if (maxZHeight !== null && !isNaN(maxZHeight)) {
                    ...
                    // FIX: Make else block required (not optional) to ensure proper capture in JavaScript regex
                    const beforeReplace = activeEndCode;
                    
                    // DEBUG: Show Z-Drop Logic section BEFORE replacement
                    const zDropIndexBefore = activeEndCode.indexOf('Z-Drop Logic');
                    if (zDropIndexBefore >= 0) {
                        console.log('[Z-DROP DEBUG] BEFORE replacement - Z-Drop Logic section (first 300 chars):');
                        console.log(activeEndCode.substring(zDropIndexBefore, zDropIndexBefore + 300));
                    } else {
                        console.warn('[Z-DROP DEBUG] WARNING: "Z-Drop Logic" comment not found in activeEndCode!');
                    }
                    
                    activeEndCode = activeEndCode.replace(/\{if\s*\(?\s*max_layer_z\s*\)?\s*>\s*([\d\.]+)\}([\s\S]*?)\{else\}([\s\S]*?)\{endif\}/g, (match, threshold, ifBlock, elseBlock) => {
                        console.log('[Z-DROP DEBUG] Conditional logic match found!');
                        console.log('[Z-DROP DEBUG] Threshold:', threshold);
                        console.log('[Z-DROP DEBUG] maxZHeight:', maxZHeight);
                        console.log('[Z-DROP DEBUG] Full match length:', match.length);
                        console.log('[Z-DROP DEBUG] ifBlock length:', ifBlock.length, 'content:', ifBlock.substring(0, 100));
                        console.log('[Z-DROP DEBUG] elseBlock length:', elseBlock.length, 'content:', elseBlock.substring(0, 100));
                        
                        const thresholdNum = parseFloat(threshold);
                        if (isNaN(thresholdNum)) {
                            console.error('[Z-DROP DEBUG] Invalid threshold:', match);
                            return match;
                        }
                        if (maxZHeight > thresholdNum) {
                            // Don't trim - preserve newlines from template structure
                            const result = ifBlock.replace(/\{max_layer_z\}/g, maxZHeight.toFixed(2))
                            ...
                            console.log('[Z-DROP DEBUG] Using IF block, result:', result);
                            return result;
                        } else {
                            // elseBlock is now guaranteed to be captured (not optional in regex)
                            console.log('[Z-DROP DEBUG] Using ELSE block');
                            console.log('[Z-DROP DEBUG] elseBlock before replace:', elseBlock);
                            // Don't trim - preserve newlines from template structure
                            const result = elseBlock.replace(/\{max_layer_z\}/g, maxZHeight.toFixed(2))
                                                          .replace(/\{max_layer_z\s*([+\-])\s*([\d\.]+)\}/g, (m, op, val) => {
                                                              const numVal = parseFloat(val);
                                                              if (isNaN(numVal)) {
                                                                  if (DEBUG_MODE) console.error(`Invalid value in expression: ${m}`);
                            ...
                            console.log('[Z-DROP DEBUG] ELSE block result:', result);
                            console.log('[Z-DROP DEBUG] ELSE block result length:', result.length);
                            return result;
                        }
                    });
                    if (beforeReplace !== activeEndCode) {
                        console.log('[Z-DROP DEBUG] Replacement occurred!');
                        
                        // DEBUG: Show Z-Drop Logic section AFTER replacement
                        const zDropIndexAfter = activeEndCode.indexOf('Z-Drop Logic');
                        if (zDropIndexAfter >= 0) {
                            console.log('[Z-DROP DEBUG] AFTER replacement - Z-Drop Logic section (first 300 chars):');
                            console.log(activeEndCode.substring(zDropIndexAfter, zDropIndexAfter + 300));
                        } else {
                            console.warn('[Z-DROP DEBUG] WARNING: "Z-Drop Logic" comment not found after replacement!');
                        }
                        
                        // Check if G1 Z1 F600 is actually in activeEndCode after replacement
                        const g1Z1Count = (activeEndCode.match(/G1 Z1 F600/g) || []).length;
                        if (g1Z1Count > 0) {
                            console.log(`[Z-DROP DEBUG] ✓ G1 Z1 F600 found in activeEndCode after replacement (${g1Z1Count} occurrences)!`);
                            
                            // Find all positions of G1 Z1 F600
                            let searchIndex = 0;
                            const positions = [];
                            while ((searchIndex = activeEndCode.indexOf('G1 Z1 F600', searchIndex)) !== -1) {
                                positions.push(searchIndex);
                                searchIndex += 10; // Move past this match
                            }
                            console.log('[Z-DROP DEBUG] G1 Z1 F600 positions:', positions);
                            
                            // Show context around each occurrence
                            positions.forEach((pos, idx) => {
                                const context = activeEndCode.substring(Math.max(0, pos - 100), pos + 100);
                                console.log(`[Z-DROP DEBUG] G1 Z1 F600 #${idx + 1} context:`, context);
                            });
                        } else {
                            console.error('[Z-DROP DEBUG] ✗ G1 Z1 F600 NOT found in activeEndCode after replacement!');
                            console.log('[Z-DROP DEBUG] Checking what was actually replaced...');
                            // Find the section around Z-Drop Logic
                            const zDropIndex = activeEndCode.indexOf('Z-Drop Logic');
                            if (zDropIndex >= 0) {
                                const section = activeEndCode.substring(zDropIndex, zDropIndex + 500);
                                console.log('[Z-DROP DEBUG] Section after replacement:', section);
                            }
                        }
                    } else {
                        console.warn('[Z-DROP DEBUG] WARNING: No replacement occurred! Conditional logic may not be in end code.');
                        // Show what we're looking for
                        const conditionalMatch = activeEndCode.match(/\{if\s*\(?\s*max_layer_z\s*\)?\s*>\s*([\d\.]+)\}/);
                        if (conditionalMatch) {
                            console.log('[Z-DROP DEBUG] Found conditional pattern but regex did not match. Pattern:', conditionalMatch[0]);
                        } else {
                            console.log('[Z-DROP DEBUG] No conditional pattern found in activeEndCode.');
                        }
                    }
                    ...
                    // FIX: Make else block required (not optional) to ensure proper capture in JavaScript regex
                    const beforeReplaceFallback = activeEndCode;
                    activeEndCode = activeEndCode.replace(/\{if\s*\(?\s*max_layer_z\s*\)?\s*>\s*([\d\.]+)\}([\s\S]*?)\{else\}([\s\S]*?)\{endif\}/g, (match, threshold, ifBlock, elseBlock) => {
                        console.log('[Z-DROP DEBUG FALLBACK] Conditional logic match found!');
                        console.log('[Z-DROP DEBUG FALLBACK] Threshold:', threshold);
                        console.log('[Z-DROP DEBUG FALLBACK] maxZHeight:', maxZHeight);
                        console.log('[Z-DROP DEBUG FALLBACK] elseBlock:', elseBlock.substring(0, 50));
                        
                        const thresholdNum = parseFloat(threshold);
                        if (isNaN(thresholdNum)) {
                            console.error('[Z-DROP DEBUG FALLBACK] Invalid threshold:', match);
                            return match;
                        }
                        if (maxZHeight > thresholdNum) {
                            // Don't trim - preserve newlines from template structure
                            ...
                            console.log('[Z-DROP DEBUG FALLBACK] Using IF block, result:', result);
                            return result;
                        } else {
                            // elseBlock is now guaranteed to be captured (not optional in regex)
                            console.log('[Z-DROP DEBUG FALLBACK] Using ELSE block');
                            // Don't trim - preserve newlines from template structure
                            const result = elseBlock.replace(/\{max_layer_z\}/g, maxZHeight.toFixed(2))
                                                          .replace(/\{max_layer_z\s*([+\-])\s*([\d\.]+)\}/g, (m, op, val) => {
                                                              const numVal = parseFloat(val);
                            ...
                            console.log('[Z-DROP DEBUG FALLBACK] ELSE block result:', result);
                            return result;
                        }
                    });
                    if (beforeReplaceFallback !== activeEndCode) {
                        console.log('[Z-DROP DEBUG FALLBACK] Replacement occurred!');
                    } else {
                        console.warn('[Z-DROP DEBUG FALLBACK] WARNING: No replacement occurred!');
                    }
                ...
                // CRITICAL FIX: Extract original T command BEFORE splitting (original start code is removed during split)
                // This ensures we use the correct T0/T1/etc from the original file instead of T255 from end code
                let originalToolCommand = null;
                const executableBlockStart = printGcodeOnly.indexOf('; EXECUTABLE_BLOCK_START');
                if (executableBlockStart !== -1) {
                    // Extract from the original executable block (before splitting)
                    const originalExecutableBlock = printGcodeOnly.substring(executableBlockStart);
                    originalToolCommand = extractOriginalToolCommand(originalExecutableBlock);
                }
                ...
                // Store the extracted original tool command
                if (originalToolCommand) {
                    variableValues['originalToolCommand'] = originalToolCommand;
                    if (DEBUG_MODE) console.log(`[T0 FIX] Preserved original tool command: T${originalToolCommand}`);
                } else {
                    // Fallback: try to extract from split structure (may not work if start code was removed)
                    if (typeof printGcodeExecutable === 'object' && printGcodeExecutable.setup) {
                        originalToolCommand = extractOriginalToolCommand(printGcodeExecutable.setup + (printGcodeExecutable.print || ''));
                    } else if (typeof printGcodeExecutable === 'string') {
                        originalToolCommand = extractOriginalToolCommand(printGcodeExecutable);
                    }
                    if (originalToolCommand) {
                        variableValues['originalToolCommand'] = originalToolCommand;
                        if (DEBUG_MODE) console.log(`[T0 FIX] Preserved original tool command (fallback): T${originalToolCommand}`);
                    } else {
                        if (DEBUG_MODE) console.warn(`[T0 FIX] Could not extract original tool command - will use initial_extruder`);
                    }
                }
                ...
                for (let i = 1; i <= loops; i++) {
                    ...
                    if (typeof printGcodeExecutable === 'object' && printGcodeExecutable.setup) {
                        ...
                        // Use Factorian's logic 100% - full start code with AMS sequence in every loop
                        ...
                        if (printGcodeExecutable.print) {
                            ...
                            // Set speed mode before print G-code
                            const speedModeInput = document.getElementById('speedMode');
                            const speedMode = speedModeInput ? parseInt(speedModeInput.value) || 100 : 100;
                            if (DEBUG_MODE) console.log(`[SPEED DEBUG] Loop ${i}: Adding M220 S${speedMode} before print G-code`);
                            parts.push(`M220 S${speedMode} ; Set speed mode\n`);
                            
                            // CRITICAL: Replace any M220 S100 in print G-code itself to prevent speed reset during printing
                            let printGcodeWithSpeed = printGcodeExecutable.print;
                            if (speedMode !== 100) {
                                const beforePrintReplace = printGcodeWithSpeed;
                                printGcodeWithSpeed = printGcodeWithSpeed.replace(/M220\s+S100\b/g, `M220 S${speedMode}`);
                                if (DEBUG_MODE && beforePrintReplace !== printGcodeWithSpeed) {
                                    const replacementCount = (printGcodeWithSpeed.match(new RegExp(`M220 S${speedMode}`, 'g')) || []).length;
                                    console.log(`[SPEED DEBUG] Loop ${i}: Replaced M220 S100 in print G-code with M220 S${speedMode}. Found ${replacementCount} M220 S${speedMode} command(s) in print G-code.`);
                                }
                            }
                            parts.push(printGcodeWithSpeed);
                            parts.push('\n');
                        }
                    } else {
                        ...
                        // Set speed mode before print G-code
                        const speedModeInputFallback = document.getElementById('speedMode');
                        const speedModeFallback = speedModeInputFallback ? parseInt(speedModeInputFallback.value) || 100 : 100;
                        if (DEBUG_MODE) console.log(`[SPEED DEBUG] Loop ${i} (fallback): Adding M220 S${speedModeFallback} before print G-code`);
                        parts.push(`M220 S${speedModeFallback} ; Set speed mode\n`);
                        parts.push(printGcodeStr);
                        parts.push('\n');
                    }
                    
                    // Add end code after each loop
                    // Use Factorian's logic 100% - full end code with unload sequence in every loop
                ...
                parts.push(`M220 S100 ; Reset to standard speed\n`);
    ...
    // --- SLUT PÅ processGcodeFile() FUNKTION ---
    ...
    /**
     * Extracts the original T (tool) command from the original start code
     * This is used to preserve the correct extruder number (T0, T1, etc.) instead of using T255 from end code
     * @param {string} executableContent - The executable block content (between FEATURE: Custom and print start)
     * @returns {string|null} The original T command value (e.g., "0", "1") or null if not found
     */
    function extractOriginalToolCommand(executableContent) {
        if (!executableContent) return null;
        
        // Find FEATURE: Custom marker
        const featureIndex = executableContent.indexOf('; FEATURE: Custom');
        if (featureIndex === -1) return null;
        
        // Get the part after FEATURE: Custom (this contains the original start code)
        const afterFeature = executableContent.substring(featureIndex + '; FEATURE: Custom'.length);
        
        // Find where the original start code ends (where print starts)
        // Look for layer markers or "Start Code End" marker
        const startCodeEndMarker = /; =+.*Start Code End.*=+/i;
        const startCodeEndMatch = afterFeature.match(startCodeEndMarker);
        const startCodeEndRegex = /; CHANGE_LAYER|;LAYER_CHANGE|; layer num\/total_layer_count|; layer |;Z_HEIGHT|;LAYER_NUM|; GEOMETRY_START/i;
        
        let originalStartCode = '';
        if (startCodeEndMatch) {
            // Extract original start code (between FEATURE: Custom and "Start Code End")
            originalStartCode = afterFeature.substring(0, startCodeEndMatch.index);
        } else {
            const splitMatch = afterFeature.match(startCodeEndRegex);
            if (splitMatch) {
                originalStartCode = afterFeature.substring(0, splitMatch.index);
            } else {
                // Fallback: use first 5000 chars (should be enough for start code)
                originalStartCode = afterFeature.substring(0, 5000);
            }
        }
        
        // Extract T command from original start code (ignore T255, T1000, and temperature values)
        // CRITICAL: Only accept T0-T3 (extruder numbers). Ignore T140, T220, etc. (temperatures in G28 Z P0 T140)
        const tMatches = [...originalStartCode.matchAll(/T(\d+)/g)];
        for (const match of tMatches) {
            const tValue = match[1];
            const tNum = parseInt(tValue, 10);
            // Only accept T0-T3 (extruder numbers)
            // Skip T255 (empty tool), T1000 (park command), and any value >= 100 (temperatures)
            if (tNum >= 0 && tNum < 4) {
                if (DEBUG_MODE) console.log(`[T0 FIX] Extracted original tool command from start code: T${tValue}`);
                return tValue;
            }
        }
        
        if (DEBUG_MODE) console.warn('[T0 FIX] Could not extract original tool command from start code');
        return null;
    }
    ...
    function extractVariableValues(gcodeContent, gcodeConfig) {
        ...
        if (!vars['initial_extruder'] || vars['initial_extruder'] === '0') {
            // CRITICAL FIX: Ignore T255 (empty tool/unload command) when searching for initial_extruder
            // T255 is used in end code for filament unload and should NOT be used as initial_extruder
            // Search for T commands but exclude T255
            const tMatches = [...gcodeContent.matchAll(/T(\d+)/g)];
            for (const match of tMatches) {
                const tValue = match[1];
                // Skip T255 (empty tool) and T1000 (park command) - these are not extruder numbers
                if (tValue !== '255' && tValue !== '1000') {
                    vars['initial_extruder'] = tValue;
                    if (DEBUG_MODE) console.log(`[T0 FIX] Found initial_extruder from T command: T${tValue}`);
                    break;
                }
            }
            // If still not found, use default '0'
            if (!vars['initial_extruder'] || vars['initial_extruder'] === '0') {
                vars['initial_extruder'] = '0';
                if (DEBUG_MODE) console.warn('[T0 FIX] Could not find valid T command (excluding T255/T1000), using default: 0');
            }
        }
    ...
    function replaceStartCodeVariables(startCode, vars) {
        ...
        // CRITICAL FIX: Get original tool command value FIRST (before expressions)
        // This ensures expressions use the correct value instead of getting "140" or "255"
        const initialExtruderForExpressions = vars['originalToolCommand'] || vars['initial_extruder'] || '0';
        
        // CRITICAL: Replace [initial_no_support_extruder] in expressions BEFORE processing expressions
        // This prevents expressions like {filament_max_volumetric_speed[initial_no_support_extruder]/2.4053*60}
        // from becoming {filament_max_volumetric_speed140/2.4053*60}
        // We do this by temporarily replacing [initial_no_support_extruder] in expressions with the numeric value
        result = result.replace(/\{([^}]*)\[initial_no_support_extruder\]([^}]*)\}/g, (match, before, after) => {
            return `{${before}[${initialExtruderForExpressions}]${after}}`;
        });
        ...
        if (!isNaN(nozzleTemp)) {
            ...
            // {nozzle_temperature_initial_layer[0]-20} (after [initial_no_support_extruder] replacement)
            result = result.replace(/\{nozzle_temperature_initial_layer\[(\d+)\]-20\}/g, (nozzleTemp - 20).toString());
            ...
            // {nozzle_temperature_initial_layer[0]} (after [initial_no_support_extruder] replacement)
            result = result.replace(/\{nozzle_temperature_initial_layer\[(\d+)\]\}/g, nozzleTemp.toString());
        }
        
        // Replace filament_max_volumetric_speed expression
        // Pattern: {filament_max_volumetric_speed[initial_extruder]/2.4053*60}
        // CRITICAL: Also handle {filament_max_volumetric_speed[0]/2.4053*60} after [initial_no_support_extruder] replacement
        if (vars['filament_max_volumetric_speed']) {
            ...
            if (!isNaN(volumetricSpeed)) {
                ...
                // Handle {filament_max_volumetric_speed[0]/2.4053*60} after [initial_no_support_extruder] replacement
                result = result.replace(/\{filament_max_volumetric_speed\[(\d+)\]\/(2\.4053|2\.4053)\*60\}/g, calculatedValue.toString());
                ...
                result = result.replace(/\{filament_max_volumetric_speed\[(\d+)\]\/(2\.4053|2\.4053)\*60\}/g, '1200');
        ...
        // CRITICAL FIX: Replace [initial_no_support_extruder] in expressions BEFORE processing expressions
        // This prevents expressions like {filament_max_volumetric_speed[initial_no_support_extruder]/2.4053*60}
        // from becoming {filament_max_volumetric_speed140/2.4053*60}
        result = result.replace(/\[initial_no_support_extruder\]/g, initialExtruderForExpressions);
        ...
        // CRITICAL: Also handle {nozzle_temperature_range_high[0]} after [initial_no_support_extruder] replacement
        if (vars['nozzle_temperature_range_high']) {
            ...
            result = result.replace(/\{nozzle_temperature_range_high\[(\d+)\]\}/g, vars['nozzle_temperature_range_high']);
        } else {
            ...
            result = result.replace(/\{nozzle_temperature_range_high\[(\d+)\]\}/g, defaultRangeHigh);
        }
        ...
        // CRITICAL FIX: Use preserved original tool command if available, otherwise use extracted initial_extruder
        // This ensures we use the correct T0/T1/etc from the original start code, not T255 from end code
        const initialExtruderFinal = vars['originalToolCommand'] || vars['initial_extruder'] || '0';
        ...
        // CRITICAL: Also use preserved original tool command here to prevent M620 S255A and T255 errors
        result = result.replace(/\[initial_no_support_extruder\]/g, initialExtruderFinal);
        
        // STEP 4: Replace M220 S100 with user's selected speed mode (if speed mode is set)
        // This ensures the speed setting is applied in the start code, not overridden
        const speedModeInput = document.getElementById('speedMode');
        if (speedModeInput) {
            const speedMode = parseInt(speedModeInput.value) || 100;
            const beforeReplace = result;
            // Replace M220 S100 with M220 S{speedMode} to apply user's speed setting
            result = result.replace(/M220\s+S100\s*;.*Reset\s+Feedrate/gi, `M220 S${speedMode} ;Reset Feedrate (Looprint: ${speedMode}% speed)`);
            // Also handle variations without comment
            result = result.replace(/M220\s+S100\s*$/gm, `M220 S${speedMode} ;Reset Feedrate (Looprint: ${speedMode}% speed)`);
            
            // DEBUG: Log if replacement occurred
            if (DEBUG_MODE && beforeReplace !== result) {
                const replacementCount = (result.match(new RegExp(`M220 S${speedMode}`, 'g')) || []).length;
                console.log(`[SPEED DEBUG] Replaced M220 S100 with M220 S${speedMode} in start code. Found ${replacementCount} M220 S${speedMode} command(s).`);
            } else if (DEBUG_MODE && speedMode !== 100) {
                console.warn(`[SPEED DEBUG] Speed mode is ${speedMode}% but no M220 S100 found to replace in start code. Speed may not be applied correctly.`);
            }
        }
        
        // A1-specific: Replace {nozzle_temperature_initial_layer[initial_no_support_extruder]} expressions
        // NOTE: [initial_no_support_extruder] is already replaced above in expressions, so these patterns should now use numeric values
        // But we still need to handle any remaining ones that weren't caught by the earlier replacement
        if (!isNaN(nozzleTemp)) {
            // Handle any remaining patterns with [initial_no_support_extruder] (shouldn't happen after fix above, but safety)
            ...
            // Handle patterns with numeric values after replacement (e.g., {nozzle_temperature_initial_layer[0]})
            result = result.replace(/\{nozzle_temperature_initial_layer\[(\d+)\]\}/g, nozzleTemp.toString());
            result = result.replace(/\{nozzle_temperature_initial_layer\[(\d+)\]-20\}/g, (nozzleTemp - 20).toString());
            result = result.replace(/\{nozzle_temperature_initial_layer\[(\d+)\]-50\}/g, (nozzleTemp - 50).toString());
        }
        
        // A1-specific: Replace {nozzle_temperature_range_high[initial_no_support_extruder]} expressions
        // NOTE: [initial_no_support_extruder] is already replaced above in expressions
        if (vars['nozzle_temperature_range_high']) {
            // Handle any remaining patterns with [initial_no_support_extruder] (shouldn't happen after fix above, but safety)
            result = result.replace(/\{nozzle_temperature_range_high\[initial_no_support_extruder\]\}/g, vars['nozzle_temperature_range_high']);
            // Handle patterns with numeric values after replacement (e.g., {nozzle_temperature_range_high[0]})
            result = result.replace(/\{nozzle_temperature_range_high\[(\d+)\]\}/g, vars['nozzle_temperature_range_high']);
        } else {
            // Fallback: use nozzle_temperature_initial_layer + 20 as safe default
            const defaultRangeHigh = vars['nozzle_temperature_initial_layer'] ? (parseInt(vars['nozzle_temperature_initial_layer'], 10) + 20).toString() : '240';
            result = result.replace(/\{nozzle_temperature_range_high\[initial_no_support_extruder\]\}/g, defaultRangeHigh);
            result = result.replace(/\{nozzle_temperature_range_high\[(\d+)\]\}/g, defaultRangeHigh);
        }
        ...
        if (vars['filament_type']) {
            ...
            // Handle patterns with numeric values after replacement (e.g., {filament_type[0]})
            result = result.replace(/\{filament_type\[(\d+)\]\}/g, vars['filament_type']);
        }
        if (vars['filament_max_volumetric_speed']) {
            const volumetricSpeed = parseFloat(vars['filament_max_volumetric_speed']);
            if (!isNaN(volumetricSpeed)) {
                ...
                // Handle patterns with numeric values after replacement (e.g., {filament_max_volumetric_speed[0]/2.4053*60})
                result = result.replace(/\{filament_max_volumetric_speed\[(\d+)\]\/(2\.4053|2\.4053)\*60\}/g, calculatedValue.toString());
            }
        }
    ...
    async function process3MFFile(file, loops) {
        console.log('[PROCESS3MF] process3MFFile() called for file:', file.name);
        // Show loading state
        ...
        try {
            ...
            // Note: originalToolCommand extraction for processGcodeFile is handled later when we parse the structure for start code
            ...
            if (maxZHeight !== null && !isNaN(maxZHeight)) {
                ...
                // Factorian uses {if (max_layer_z ) > 41} with space - handle both formats
                // FIX: Make else block required (not optional) to ensure proper capture in JavaScript regex
                const beforeReplaceGcode = activeEndCode;
                
                // DEBUG: Show Z-Drop Logic section BEFORE replacement
                const zDropIndexBeforeGcode = activeEndCode.indexOf('Z-Drop Logic');
                if (zDropIndexBeforeGcode >= 0) {
                    console.log('[Z-DROP DEBUG GCODE] BEFORE replacement - Z-Drop Logic section (first 300 chars):');
                    console.log(activeEndCode.substring(zDropIndexBeforeGcode, zDropIndexBeforeGcode + 300));
                } else {
                    console.warn('[Z-DROP DEBUG GCODE] WARNING: "Z-Drop Logic" comment not found in activeEndCode!');
                }
                
                activeEndCode = activeEndCode.replace(/\{if\s*\(?\s*max_layer_z\s*\)?\s*>\s*([\d\.]+)\}([\s\S]*?)\{else\}([\s\S]*?)\{endif\}/g, (match, threshold, ifBlock, elseBlock) => {
                    console.log('[Z-DROP DEBUG GCODE] Conditional logic match found!');
                    console.log('[Z-DROP DEBUG GCODE] Threshold:', threshold);
                    console.log('[Z-DROP DEBUG GCODE] maxZHeight:', maxZHeight);
                    console.log('[Z-DROP DEBUG GCODE] Full match length:', match.length);
                    console.log('[Z-DROP DEBUG GCODE] ifBlock length:', ifBlock.length, 'content:', ifBlock.substring(0, 100));
                    console.log('[Z-DROP DEBUG GCODE] elseBlock length:', elseBlock.length, 'content:', elseBlock.substring(0, 100));
                    
                    const thresholdNum = parseFloat(threshold);
                    if (isNaN(thresholdNum)) {
                        console.error('[Z-DROP DEBUG GCODE] Invalid threshold:', match);
                        return match;
                    }
                    if (maxZHeight > thresholdNum) {
                        const result = ifBlock.replace(/\{max_layer_z\}/g, maxZHeight.toFixed(2))
                        ...
                        console.log('[Z-DROP DEBUG GCODE] Using IF block, result:', result);
                        return result;
                    } else {
                        // elseBlock is now guaranteed to be captured (not optional in regex)
                        console.log('[Z-DROP DEBUG GCODE] Using ELSE block');
                        // Don't trim - preserve newlines from template structure
                        const result = elseBlock.replace(/\{max_layer_z\}/g, maxZHeight.toFixed(2))
                                                  .replace(/\{max_layer_z\s*([+\-])\s*([\d\.]+)\}/g, (m, op, val) => {
                                                      const numVal = parseFloat(val);
                        ...
                        console.log('[Z-DROP DEBUG GCODE] ELSE block result:', result);
                        console.log('[Z-DROP DEBUG GCODE] ELSE block result length:', result.length);
                        console.log('[Z-DROP DEBUG GCODE] Returning result to replace conditional block');
                        return result;
                    }
                });
                if (beforeReplaceGcode !== activeEndCode) {
                    console.log('[Z-DROP DEBUG GCODE] Replacement occurred!');
                    
                    // DEBUG: Show Z-Drop Logic section AFTER replacement
                    const zDropIndexAfterGcode = activeEndCode.indexOf('Z-Drop Logic');
                    if (zDropIndexAfterGcode >= 0) {
                        console.log('[Z-DROP DEBUG GCODE] AFTER replacement - Z-Drop Logic section (first 300 chars):');
                        console.log(activeEndCode.substring(zDropIndexAfterGcode, zDropIndexAfterGcode + 300));
                    } else {
                        console.warn('[Z-DROP DEBUG GCODE] WARNING: "Z-Drop Logic" comment not found after replacement!');
                    }
                    
                    // Check if G1 Z1 F600 is actually in activeEndCode after replacement
                    const g1Z1CountGcode = (activeEndCode.match(/G1 Z1 F600/g) || []).length;
                    if (g1Z1CountGcode > 0) {
                        console.log(`[Z-DROP DEBUG GCODE] ✓ G1 Z1 F600 found in activeEndCode after replacement (${g1Z1CountGcode} occurrences)!`);
                        ...
                        console.log('[Z-DROP DEBUG GCODE] G1 Z1 F600 positions:', positions);
                        
                        // Show context around each occurrence
                        positions.forEach((pos, idx) => {
                            const context = activeEndCode.substring(Math.max(0, pos - 100), pos + 100);
                            console.log(`[Z-DROP DEBUG GCODE] G1 Z1 F600 #${idx + 1} context:`, context);
                        });
                    } else {
                        console.error('[Z-DROP DEBUG GCODE] ✗ G1 Z1 F600 NOT found in activeEndCode after replacement!');
                        console.log('[Z-DROP DEBUG GCODE] Checking what was actually replaced...');
                        // Find the section around Z-Drop Logic
                        const zDropIndex = activeEndCode.indexOf('Z-Drop Logic');
                        if (zDropIndex >= 0) {
                            const section = activeEndCode.substring(zDropIndex, zDropIndex + 500);
                            console.log('[Z-DROP DEBUG GCODE] Section after replacement:', section);
                        }
                    }
                } else {
                    console.warn('[Z-DROP DEBUG GCODE] WARNING: No replacement occurred! Conditional logic may not be in end code.');
                    // Show what we're looking for
                    const conditionalMatch = activeEndCode.match(/\{if\s*\(?\s*max_layer_z\s*\)?\s*>\s*([\d\.]+)\}/);
                    if (conditionalMatch) {
                        console.log('[Z-DROP DEBUG GCODE] Found conditional pattern but regex did not match. Pattern:', conditionalMatch[0]);
                    } else {
                        console.log('[Z-DROP DEBUG GCODE] No conditional pattern found in activeEndCode.');
                    }
                }
                // Now replace remaining {max_layer_z} variables (outside conditionals)
                activeEndCode = activeEndCode.replace(/\{max_layer_z\}/g, maxZHeight.toFixed(2));
                ...
                // FIX: Make else block required (not optional) to ensure proper capture in JavaScript regex
                // Factorian uses {if (max_layer_z ) > 41} with space - handle both formats
                const beforeReplaceGcodeFallback = activeEndCode;
                activeEndCode = activeEndCode.replace(/\{if\s*\(?\s*max_layer_z\s*\)?\s*>\s*([\d\.]+)\}([\s\S]*?)\{else\}([\s\S]*?)\{endif\}/g, (match, threshold, ifBlock, elseBlock) => {
                    console.log('[Z-DROP DEBUG GCODE FALLBACK] Conditional logic match found!');
                    console.log('[Z-DROP DEBUG GCODE FALLBACK] Threshold:', threshold);
                    console.log('[Z-DROP DEBUG GCODE FALLBACK] maxZHeight:', maxZHeight);
                    console.log('[Z-DROP DEBUG GCODE FALLBACK] elseBlock:', elseBlock.substring(0, 50));
                    
                    const thresholdNum = parseFloat(threshold);
                    if (isNaN(thresholdNum)) {
                        console.error('[Z-DROP DEBUG GCODE FALLBACK] Invalid threshold:', match);
                        return match;
                    }
                    if (maxZHeight > thresholdNum) {
                        // Don't trim - preserve newlines from template structure
                        ...
                        console.log('[Z-DROP DEBUG GCODE FALLBACK] Using IF block, result:', result);
                        return result;
                    } else {
                        // elseBlock is now guaranteed to be captured (not optional in regex)
                        console.log('[Z-DROP DEBUG GCODE FALLBACK] Using ELSE block');
                        // Don't trim - preserve newlines from template structure
                        const result = elseBlock.replace(/\{max_layer_z\}/g, maxZHeight.toFixed(2))
                                                  .replace(/\{max_layer_z\s*([+\-])\s*([\d\.]+)\}/g, (m, op, val) => {
                                                      const numVal = parseFloat(val);
                        ...
                        console.log('[Z-DROP DEBUG GCODE FALLBACK] ELSE block result:', result);
                        return result;
                    }
                });
                if (beforeReplaceGcodeFallback !== activeEndCode) {
                    console.log('[Z-DROP DEBUG GCODE FALLBACK] Replacement occurred!');
                } else {
                    console.warn('[Z-DROP DEBUG GCODE FALLBACK] WARNING: No replacement occurred!');
                }
                // Show warning to user
                showStatus("validationWarning", "warning", 
                    "Warning: Could not extract model height (max_z_height) from G-code header. " +
            ...
            if (executableBlockStart !== -1) {
                ...
                if (featureIndex !== -1) {
                    ...
                    // ROBUST REGEX: Leder efter standard Bambu/Orca lag-skift eller Z-højde markører
                    // Prioriteret liste af markører der indikerer starten af faktisk print
                    // CRITICAL: Layer markers must come FIRST - they are the most reliable indicators
            ...
            // Store the extracted original tool command (extracted BEFORE splitting)
            if (originalToolCommand) {
                variableValues['originalToolCommand'] = originalToolCommand;
                if (DEBUG_MODE) console.log(`[T0 FIX] Preserved original tool command: T${originalToolCommand}`);
            } else {
            ...
            for (let i = 1; i <= loops; i++) {
                ...
                if (typeof printGcodeExecutable === 'object' && printGcodeExecutable.setup) {
                    ...
                    if (printGcodeExecutable.print) {
                        ...
                        // Set speed mode before print G-code
                        const speedModeInputGcode = document.getElementById('speedMode');
                        const speedModeGcode = speedModeInputGcode ? parseInt(speedModeInputGcode.value) || 100 : 100;
                        if (DEBUG_MODE) console.log(`[SPEED DEBUG] Loop ${i} (3MF): Adding M220 S${speedModeGcode} before print G-code`);
                        parts.push(`M220 S${speedModeGcode} ; Set speed mode\n`);
                        
                        // CRITICAL: Replace any M220 S100 in print G-code itself to prevent speed reset during printing
                        let printGcodeWithSpeed3MF = printGcodeExecutable.print;
                        if (speedModeGcode !== 100) {
                            const beforePrintReplace3MF = printGcodeWithSpeed3MF;
                            printGcodeWithSpeed3MF = printGcodeWithSpeed3MF.replace(/M220\s+S100\b/g, `M220 S${speedModeGcode}`);
                            if (DEBUG_MODE && beforePrintReplace3MF !== printGcodeWithSpeed3MF) {
                                const replacementCount3MF = (printGcodeWithSpeed3MF.match(new RegExp(`M220 S${speedModeGcode}`, 'g')) || []).length;
                                console.log(`[SPEED DEBUG] Loop ${i} (3MF): Replaced M220 S100 in print G-code with M220 S${speedModeGcode}. Found ${replacementCount3MF} M220 S${speedModeGcode} command(s) in print G-code.`);
                            }
                        }
                        parts.push(printGcodeWithSpeed3MF);
                        parts.push('\n');
                    }
                } else {
                    // Fallback: if structure is different, use as string
                    ...
                    if (!printGcodeStr.includes('machine: P1S') && !printGcodeStr.includes('FactorianDesigns')) {
                        ...
                        parts.push(startCodeWithVars);
                        parts.push('\n');
                        // Set speed mode before print G-code (fallback path)
                        const speedModeInputGcodeFallback = document.getElementById('speedMode');
                        const speedModeGcodeFallback = speedModeInputGcodeFallback ? parseInt(speedModeInputGcodeFallback.value) || 100 : 100;
                        if (DEBUG_MODE) console.log(`[SPEED DEBUG] Loop ${i} (3MF fallback): Adding M220 S${speedModeGcodeFallback} before print G-code`);
                        parts.push(`M220 S${speedModeGcodeFallback} ; Set speed mode\n`);
                        parts.push(printGcodeExecutable);
                        parts.push('\n');
                    } else {
                        // Start code already present in file - can't insert speed mode in middle, skip it
                        parts.push(printGcodeExecutable);
                        parts.push('\n');
                    }
                }
                ...
                // DEBUG: Check if G1 Z1 F600 is still in activeEndCode before pushing
                if (activeEndCode.includes('G1 Z1 F600')) {
                    console.log('[PROCESS3MF DEBUG] ✓ G1 Z1 F600 STILL in activeEndCode before parts.push()');
                } else {
                    console.error('[PROCESS3MF DEBUG] ✗ G1 Z1 F600 MISSING from activeEndCode before parts.push()!');
                    // Find Z-Drop Logic section
                    const zDropIndex = activeEndCode.indexOf('Z-Drop Logic');
                    if (zDropIndex >= 0) {
                        const section = activeEndCode.substring(zDropIndex, zDropIndex + 500);
                        console.log('[PROCESS3MF DEBUG] Section around Z-Drop Logic:', section);
                    }
                }
            ...
            // DEBUG: Check if G1 Z1 F600 is in the final modifiedGcode
            const g1Z1Count = (modifiedGcode.match(/G1\s+Z1\s+F600/g) || []).length;
            if (g1Z1Count > 0) {
                console.log(`[PROCESS3MF DEBUG] ✓ G1 Z1 F600 found in final modifiedGcode (${g1Z1Count} occurrences)`);
            } else {
                console.error('[PROCESS3MF DEBUG] ✗ G1 Z1 F600 MISSING from final modifiedGcode!');
                // Find first Z-Drop Logic section
                const zDropIndex = modifiedGcode.indexOf('Z-Drop Logic');
                if (zDropIndex >= 0) {
                    const section = modifiedGcode.substring(zDropIndex, zDropIndex + 500);
                    console.log('[PROCESS3MF DEBUG] Section around first Z-Drop Logic in modifiedGcode:', section);
                }
            }
    ...
    // --- SLUT PÅ process3MFFile() FUNKTION ---
...
`;
    ...
    function generateA1PushGcode() {
        ...
        return `M220 S100 ; Reset to standard speed for safe push-off
...
G1 Z{max_layer_z +5}	; lift Z into positive again dont erase`;
    ...
    /**
     * Removes AMS material switching sequence from A1/A1Mini start code
     * This is used for subsequent loops in looped prints where material is already loaded
     * 
     * The AMS sequence pattern (M620...M621 block):
     * - M620 M (enable remap)
     * - M620 S[initial_no_support_extruder]A (switch material if AMS exist)
     * - M620.1 E F... T... (flush sequence - the "poop" part)
     * - M621 S[initial_no_support_extruder]A (end material switch)
     * 
     * @param {string} startCode - The start code to process
     * @returns {string} - Start code with AMS sequence removed
     */
    function removeA1AMSSequence(startCode) {
        if (!startCode) return startCode;
        
        // Pattern to match the AMS material switching sequence (M620...M621 block)
        // This handles both variable format [initial_no_support_extruder] and replaced format (0, 1, 2, 3)
        // Uses non-greedy matching to stop at first M621 S[...]A
        // Pattern matches from "M620 M" to "M621 S[...]A"
        
        // Most flexible pattern: Match M620 M...M621 S[...]A (handles any format)
        // This matches:
        // - Variable format: M620 S[initial_no_support_extruder]A ... M621 S[initial_no_support_extruder]A
        // - Replaced format: M620 S0A ... M621 S0A (or S1A, S2A, etc.)
        const amsPattern = /M620 M[\s\S]*?M621 S[^\s\n]+\s*\n?/g;
        
        let modifiedCode = startCode.replace(amsPattern, '');
        
        if (DEBUG_MODE && modifiedCode !== startCode) {
            const removedLength = startCode.length - modifiedCode.length;
            console.log(`[A1 AMS SEQUENCE] Removed AMS material switching sequence (${removedLength} chars)`);
        } else if (DEBUG_MODE && modifiedCode === startCode) {
            // Check if AMS sequence exists but pattern didn't match
            if (startCode.includes('M620 M') || startCode.includes('M621 S')) {
                console.warn('[A1 AMS SEQUENCE] AMS sequence pattern found but regex did not match - may need pattern adjustment');
            }
        }
    ...
    /**
     * Removes filament unload sequence from A1/A1Mini end code
     * This is used for intermediate loops in looped prints where filament should stay loaded
     * 
     * The unload sequence pattern:
     * - Comment "; pull back filament to AMS"
     * - M620 S255
     * - G1 X commands (with various X values and optional F speeds)
     * - T255
     * - More G1 X commands (wipe movements)
     * - M621 S255
     * 
     * @param {string} endCode - The end code to process
     * @returns {string} - End code with unload sequence removed
     */
    function removeA1FilamentUnload(endCode) {
        if (!endCode) return endCode;
        
        // Pattern to match the unload sequence from comment to M621 S255
        // This handles both A1 and A1Mini formats (different X values)
        // Uses non-greedy matching to stop at M621 S255
        const unloadPattern = /; pull back filament to AMS[\s\S]*?M621 S255\s*\n?/g;
        
        let modifiedCode = endCode.replace(unloadPattern, '');
        
        if (DEBUG_MODE && modifiedCode !== endCode) {
            const removedLength = endCode.length - modifiedCode.length;
            console.log(`[A1 FILAMENT UNLOAD] Removed unload sequence (${removedLength} chars)`);
        } else if (DEBUG_MODE && modifiedCode === endCode) {
            // Check if unload sequence exists but pattern didn't match
            if (endCode.includes('; pull back filament to AMS') || endCode.includes('M620 S255')) {
                console.warn('[A1 FILAMENT UNLOAD] Unload sequence pattern found but regex did not match - may need pattern adjustment');
            }
        }
...
`;
    ...
    function getX1EndCode(offset, temp, xStart, xLeft, xCenter, xRight) {
        ...
        // 1. Retract & move to safe position + Filament retraction & cutter sequence (FØR cooldown)
        ...
        // Step 1: Retract, move to safe position, and cutter sequence (FØR cooldown)
...
`;
    ...
    function updatePreview() {
        ...
        if (currentPrinterModel === 'A1' || currentPrinterModel === 'A1Mini') {
            ...
            if (bedPreviewModeText) {
                if (currentPrinterModel === 'A1Mini') {
                    bedPreviewModeText.innerHTML = 'The <strong style="color: var(--accent);">green border</strong> shows the active print area (X0 to X180). Print head parks at X-13 after each print (parking position). Wiggle sweep extends to X-13.<br><span style="font-size: 0.85em; color: var(--danger);">⚠️ A1 Mini requires 50mm+ clearance to the left of the printer for wiggle sweep (shown in red zone).</span>';
                } else {
                    bedPreviewModeText.innerHTML = 'The <strong style="color: var(--accent);">green border</strong> shows the active print area (X0 to X256). Print head parks at X-48 after each print (parking position). Wiggle sweep extends to X-48.<br><span style="font-size: 0.85em; color: var(--muted);">ℹ️ A1 print head is parked at X-48 (no clearance needed). A1 Mini users should ensure 50mm+ clearance to the left.</span>';
                }
            }
            if (sweepModeText) {
                if (currentPrinterModel === 'A1Mini') {
                    sweepModeText.textContent = '(Full-width wiggle sweep: X180 → X-13)';
                } else {
                    sweepModeText.textContent = '(Full-width wiggle sweep: X256 → X-48)';
                }
            }
        } else {
...
• Moves hotend to safe position
• Lowers bed to ${bedZ.toFixed(2)}mm (Factorian method: ${modelHeight.toFixed(2)}mm - 30mm)
• Performs push-off from ${directionName} side (outer lane first, then inner lane)
...
; - Else: G1 Z1.0
; 
; For ${modelHeight.toFixed(2)}mm model: ${modelHeight > 41 ? `${modelHeight.toFixed(2)} > 41` : `${modelHeight.toFixed(2)} <= 41`} → G1 Z${bedZ.toFixed(2)}
...
`;
            ...
            ; - Else: G1 Z1
            ; 
            ; For ${modelHeight.toFixed(2)}mm model: ${modelHeight > 31 ? `${modelHeight.toFixed(2)} > 31` : `${modelHeight.toFixed(2)} <= 31`} → G1 Z${bedZ.toFixed(2)}
            ; This lowers bed from ${modelHeight.toFixed(2)}mm to ${bedZ.toFixed(2)}mm (${(modelHeight - bedZ).toFixed(2)}mm lower)
...
// Instructions modal and Google Translate widget logic moved to reusable components:
// - src/js/components/instructions-modal.js
// - src/js/components/translate-widget.js
